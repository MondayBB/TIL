...
const introRightBan = document.querySelector(".introRightBan");
const introRightBanDiv = introRightBan.querySelectorAll("div");
...
let currentTime = new Date().getTime();
  ...
  // 행외시 이벤트 시간, 변시 이벤트 시간
const eventStartTime = new Date(2022,7-1,18,10,0).getTime();
const eventEndTime = new Date(2022,7-1,25,17,1).getTime();

  // 경찰승진 이벤트 시간
const policeStartTime = new Date(2022,7-1,19,10,0).getTime();
const policeEndTime = new Date(2022,8-1,1,17,1).getTime();
...
// 인트로 우측배너 시작, 종료
function eventRightBan(eventStartTime, eventEndTime, bannerIndex){
  if(currentTime >= eventStartTime && currentTime < eventEndTime){
    introRightBanDiv[bannerIndex].classList.remove("hide");
    introRightBanDiv[bannerIndex].classList.add("show");
  }else{
    introRightBanDiv[bannerIndex].classList.remove("show");
    introRightBanDiv[bannerIndex].classList.add("hide");
  }
}
eventRightBan(policeStartTime, policeEndTime, 0);
eventRightBan(eventStartTime, eventEndTime, 1);
...
function introReload(){
  ...
  if((currentTime >= eventStartTime && currentTime < eventStartTime+7500) || (currentTime >= eventEndTime && currentTime < eventEndTime+7500) || (currentTime >= policeStartTime && currentTime < policeStartTime+7500) || (currentTime >= policeEndTime && currentTime < policeEndTime+7500)){
    location.reload();
  }
}
